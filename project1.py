...
# 로그인 화면의 stForm 전용 워터마크 CSS 분리 주입 (f-string 적용하여 base64 이미지 동적 결합)
st.markdown(f"""
    <style>
    div[data-testid="stForm"] {{
        max-width: 450px !important; margin: 4% auto !important; background-color: rgba(255, 255, 255, 0.96) !important;
        background-image: linear-gradient(rgba(255, 255, 255, 0.91), rgba(255, 255, 255, 0.91)), url('data:image/png;base64,{logo_base64}') !important;
        ...
        with st.form("login_form", border=True):
            st.markdown(f"""
                <div style="text-align: center; margin-bottom: 1rem;">
                    <img src="data:image/png;base64,{logo_base64}" style="width: 140px; height: 140px; object-fit: contain;" />
                </div>
